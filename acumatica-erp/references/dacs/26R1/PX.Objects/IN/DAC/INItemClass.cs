...
namespace PX.Objects.IN
{
	...
	public class INItemClass : PXBqlTable, IBqlTable, PX.SM.IIncludable
	{
		public const string Dimension = "INITEMCLASS";
		public class dimension() : BqlString.Constant<dimension>(Dimension);

		#region Keys
		public class PK : PrimaryKeyOf<INItemClass>.By<itemClassID>
		{
			public static INItemClass Find(PXGraph graph, int? itemClassID, PKFindOptions options = PKFindOptions.None) => FindBy(graph, itemClassID, options);
		...
		[PXUIVisible(typeof(Where<planningMethod, Equal<INPlanningMethod.inventoryReplenishment>, And<stkItem, Equal<True>>>))]
		...
		[PXDefault(typeof(SelectParentItemClass), SourceField = typeof(commodityCodeType), PersistingCheck = PXPersistingCheck.Nothing)]
		...
		[PXFormula(typeof(IIf<Where<stkItem.IsEqual<True>>, InventoryItem.postToExpenseAccount.purchases, InventoryItem.postToExpenseAccount>))]
		...
		public abstract class postToExpenseAccount : BqlBool.Field<postToExpenseAccount> { }
		#endregion
		#region Audit Fields
		#region CreatedByID
		[PXDBCreatedByID]
		...
		[PXUIField(Visible = false, Visibility = PXUIVisibility.Invisible)]
		[PXDimension(Dimension, ParentSelect = typeof(Select<INItemClass>), ParentValueField = typeof(itemClassCD), AutoNumbering = false)]
		...
		[PXDefault(INPlanningMethod.None)]
		[INPlanningMethod.List]
		[PXUIVisible(typeof(Where<stkItem, Equal<True>>))]
		...
		[PXDefault(PreferredItemClassesList.AllClasses)]
		...
		/// <exclude/>
		[PXDBString(1, IsFixed = true, IsUnicode = false)]
		[PXDefault(PriceOfSuggestedItemsList.AnyPrice)]
	...
	public class INDemandCalculation
	{
		...
		public class itemClassSettings() : BqlString.Constant<itemClassSettings>(ItemClassSettings);
		public class hardDemand() : BqlString.Constant<hardDemand>(HardDemand);
	}

	// Used in AR674000.rpx
	public class FilterItemByClass : PXBqlTable, IBqlTable
	{
		#region ItemClassID
		[PXDBInt]
