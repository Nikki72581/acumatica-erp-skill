...
namespace PX.Objects.IN
{
	...
	[PXCacheName(Messages.Warehouse, PXDacType.Catalogue, CacheGlobal = true)]
	public class INSite : PXBqlTable, IBqlTable, PX.SM.IIncludable
	{
		#region Keys
		public class PK : PrimaryKeyOf<INSite>.By<siteID>.Dirty
		{
			public static INSite Find(PXGraph graph, int? siteID, PKFindOptions options = PKFindOptions.None) =>
				FindBy(graph, siteID, (options == PKFindOptions.None && (siteID ?? 0) <= 0) ? PKFindOptions.IncludeDirty : options);
		}
		public class UK : PrimaryKeyOf<INSite>.By<siteCD>
		{
			public static INSite Find(PXGraph graph, string siteCD, PKFindOptions options = PKFindOptions.None) => FindBy(graph, siteCD, options);
		}
		...
		public abstract class branchID : BqlInt.Field<branchID>
		{
            ...
            public class PreventEditIfSOExists: PreventEditOf<branchID>.On<INSiteMaint>.IfExists<
					SelectFrom<INCostStatus>
					.Where<INCostStatus.siteID.IsEqual<siteID.FromCurrent>
                        .And<INCostStatus.qtyOnHand.IsNotEqual<decimal0>>>>
            {
				protected override string CreateEditPreventingReason(GetEditPreventingReasonArgs arg, object firstPreventingEntity, string fieldName, string currentTableName, string foreignTableName)
					=> PXMessages.Localize(Messages.BranchCannotBeChanged);
			}

			public class PreventEditIfHistoryExists : PreventEditOf<branchID>.On<INSiteMaint>.IfExists<
					SelectFrom<INItemStats>
					.Where<INItemStats.siteID.IsEqual<siteID.FromCurrent>>>
			{
				...
				protected override string CreateEditPreventingReason(GetEditPreventingReasonArgs arg, object firstPreventingEntity, string fieldName, string currentTableName, string foreignTableName)
				{
					INSite site = arg.Row as INSite;
					var newBranch = Branch.PK.Find(Base, arg.NewValue as int?);

					return (site?.BaseCuryID == newBranch.BaseCuryID) ? null :
						PXMessages.LocalizeFormat(Messages.CannotChangeTheBranchIfHistoryExists, site?.SiteCD, site?.BaseCuryID);
				}
			}

			public class ExtensionSort
				: SortExtensionsBy<ExtensionOrderFor<INSiteMaint>
					.FilledWith<
						PreventEditIfHistoryExists,
						PreventEditIfSOExists>>
			{
			}
		}
		#endregion
		...
		[PXFormula(typeof(Selector<branchID, Branch.baseCuryID>))]
		[PXUIField(DisplayName = "Base Currency ID", Enabled = false)]
		public virtual string BaseCuryID { get; set; }
		public abstract class baseCuryID : BqlString.Field<baseCuryID> { }
		#endregion
		#region AddressID
		[PXDBInt]
		[PXDBChildIdentity(typeof(Address.addressID))]
		[PXForeignReference(typeof(FK.Address))]
		...
		[PXForeignReference(typeof(FK.Contact))]
		...
		public abstract class avgDefaultCost : BqlString.Field<avgDefaultCost>
		{
			...
			public class averageCost() : BqlString.Constant<averageCost>(AverageCost);
			public class lastCost() : BqlString.Constant<lastCost>(LastCost);
		}
		#endregion
		...
		#endregion
		#region NonStockPickingLocationID
		// Acuminator disable once PX1007 NoXmlCommentForPublicEntityOrDacProperty to be documented later
		[Location(typeof(siteID), DisplayName = "Non-Stock Location", DescriptionField = typeof(INLocation.descr), DirtyRead = true)]
		[PXRestrictor(typeof(Where<INLocation.active.IsEqual<True>>), Messages.LocationIsNotActive)]
		public virtual Int32? NonStockPickingLocationID { get; set; }
		public abstract class nonStockPickingLocationID : BqlInt.Field<nonStockPickingLocationID> { }
		#endregion
		#region CarrierFacility
		public abstract class carrierFacility : BqlString.Field<carrierFacility> { }
		...
		public class main() : BqlString.Constant<main>(Main);
	}
	...
	public class INLocationValid
	{
		...
		public class validate() : BqlString.Constant<validate>(Validate);
		public class noValidate() : BqlString.Constant<noValidate>(NoValidate);
		public class warn() : BqlString.Constant<warn>(Warn);
	}
	#endregion
