...
namespace PX.Objects.AR
{
	...
	public partial class ARTran : PXBqlTable, IBqlTable, IHasMinGrossProfit, DR.Descriptor.IDocumentLine, ISortOrder, ILSPrimary, IAccountable, IN.RelatedItems.ISubstitutableLine, IItemPlanSource
	{
		#region Keys
		public class PK : PrimaryKeyOf<ARTran>.By<tranType, refNbr, lineNbr>
		{
			public static ARTran Find(PXGraph graph, string tranType, string refNbr, int? lineNbr, PKFindOptions options = PKFindOptions.None) => FindBy(graph, tranType, refNbr, lineNbr, options);
		}

		public static class FK
		{
			public class Document : AR.ARRegister.PK.ForeignKeyOf<ARTran>.By<tranType, refNbr> { }
			public class Invoice : AR.ARInvoice.PK.ForeignKeyOf<ARTran>.By<tranType, refNbr> { }
			public class Payment : AR.ARPayment.PK.ForeignKeyOf<ARTran>.By<tranType, refNbr> { }
			public class CashSale : Standalone.ARCashSale.PK.ForeignKeyOf<ARTran>.By<tranType, refNbr> { }
			public class SOInvoice : SO.SOInvoice.PK.ForeignKeyOf<ARTran>.By<tranType, refNbr> { }
			...
			public class Tax : TX.Tax.PK.ForeignKeyOf<ARTran>.By<taxID> { }
			public class TaxCategory : TX.TaxCategory.PK.ForeignKeyOf<ARTran>.By<taxCategoryID> { }
			public class SOOrder : SO.SOOrder.PK.ForeignKeyOf<ARTran>.By<sOOrderType, sOOrderNbr> { }
			public class SOOrderLine : SO.SOLine.PK.ForeignKeyOf<ARTran>.By<sOOrderType, sOOrderNbr, sOOrderLineNbr> { }
			public class SOOrderShipment : SO.SOOrderShipment.UK.ForeignKeyOf<ARTran>.By<sOShipmentType, sOShipmentNbr, sOOrderType, sOOrderNbr> { }
			public class Customer : AR.Customer.PK.ForeignKeyOf<ARTran>.By<customerID> { }

			public class InventoryItem : IN.InventoryItem.PK.ForeignKeyOf<ARTran>.By<inventoryID> { }
			public class SubItem : IN.INSubItem.PK.ForeignKeyOf<ARTran>.By<subItemID> { }
			...
			public class ExpenseAccrualAccount : GL.Account.PK.ForeignKeyOf<ARTran>.By<expenseAccrualAccountID> { }
			public class ExpenseAccrualSubaccount : GL.Sub.PK.ForeignKeyOf<ARTran>.By<expenseAccrualSubID> { }

			public class ExpenseAccount : GL.Account.PK.ForeignKeyOf<ARTran>.By<expenseAccountID> { }
			public class ExpenseSubaccount : GL.Sub.PK.ForeignKeyOf<ARTran>.By<expenseSubID> { }
			public class DeferralCode : DR.DRDeferredCode.PK.ForeignKeyOf<ARTran>.By<deferredCode> { }
			public class Site : INSite.PK.ForeignKeyOf<ARTran>.By<siteID> { }

			[Obsolete("This foreign key is obsolete and is going to be removed in 2021R1. Use SOOrder instead.")]
			public class Order : SOOrder { }

			public class BlanketOrderLink : SO.SOBlanketOrderLink.PK.ForeignKeyOf<ARTran>.By<blanketType, blanketNbr, sOOrderType, sOOrderNbr> { }
		}
		#endregion
		#region Selected
		...
		[SO.CopyLinkToSOInvoice(typeof(SO.SOInvoice.tranCntr), typeof(curyExtPrice),
			new Type[] { typeof(sOOrderType), typeof(sOOrderNbr) },
			new Type[] { typeof(SO.SOInvoice.sOOrderType), typeof(SO.SOInvoice.sOOrderNbr) })]
		...
		public abstract class sOOrderLineOperation : PX.Data.BQL.BqlString.Field<sOOrderLineOperation>
		{
		}
		...
		#endregion
		#region SOShipmentLineGroupNbr
		/// <inheritdoc cref="SOShipmentLineGroupNbr"/>
		public abstract class sOShipmentLineGroupNbr : Data.BQL.BqlInt.Field<sOShipmentLineGroupNbr> { }
		/// <summary>
		/// Number of the group of shipment lines referencing the same <see cref="SO.SOLine">sales order line</see> which
		/// are invoiced together in the single <see cref="ARTran">invoice line</see>.
		/// </summary>
		[PXDBInt]
		public virtual int? SOShipmentLineGroupNbr
		{
			get;
			set;
		}
		...
		[PXDBLong()]
		[CurrencyInfo(typeof(ARRegister.curyInfoID), Required = true, PopulateParentCuryInfoID = true)]
		...
		public abstract class skipLineDiscounts : Data.BQL.BqlBool.Field<skipLineDiscounts> { }
		/// <summary>
		...
		[PXDBBool()]
		[PXDefault(typeof(
			InventoryItem.postToExpenseAccount.FromSelectorOf<ARTran.inventoryID>.
		...
		#endregion
		#region CostBasisNull
		[PXString(1, IsFixed = true)]
		[PXUIField(DisplayName = "Cost Based On", Enabled = false, Visible = false)]
		[CostBasisOption.List]
		[PXFormula(typeof(IIf<costBasis.IsEqual<CostBasisOption.undefinedCostBasis>, Null, costBasis>))]
		public virtual string CostBasisNull { get; set; }
		public abstract class costBasisNull : PX.Data.BQL.BqlString.Field<costBasisNull> { }
		#endregion
		#region CuryInventoryID
		public abstract class curyInventoryID : PX.Data.BQL.BqlString.Field<curyInventoryID> { }

		[PXInt]
		[PXSelector(typeof(Search<InventoryItemCurySettings.inventoryID,
			Where<InventoryItemCurySettings.inventoryID, Equal<inventoryID.FromCurrent>,
				And<InventoryItemCurySettings.curyID, Equal<Current<AccessInfo.baseCuryID>>>>>),
			ValidateValue = false)]
		[PXFormula(typeof(inventoryID))]
		...
		[PXDBBaseCury()]
		[PXFormula(typeof(Switch<
			Case<Where<accrueCost, Equal<True>, And<costBasis, Equal<CostBasisOption.standardCost>>>,
				Mult<ARTran.baseQty, Selector<curyInventoryID, InventoryItemCurySettings.stdCost>>>,
			decimal0>))]
		[PXDefault(TypeCode.Decimal, "0.0")]
		...
		[ARDocType.List()]
		[PXUIField(DisplayName = "Orig. Doc. Type", Visibility = PXUIVisibility.Visible, Visible = false, Enabled = false)]
		...
		[PXUIField(DisplayName = "Orig. Ref. Nbr.", Visibility = PXUIVisibility.Visible, Visible = false, Enabled = false)]
		...
		/// </summary>
		[Common.Attributes.PXDBRestrictionBool(typeof(released))]
		...
		[PXSelector(
			typeof(Search<DR.DRSchedule.scheduleID, 
				Where<DR.DRSchedule.bAccountID, Equal<Current<ARInvoice.customerID>>,
					And<Where2<
						Where<Current<ARTran.tranType>, NotEqual<ARDocType.creditMemo>, And<Current<ARTran.tranType>, NotEqual<ARDocType.debitMemo>>>,
						Or<Where<DR.DRSchedule.docType, NotEqual<Current<ARTran.tranType>>,
							And<Where<Current<ARTran.tranType>, Equal<ARDocType.creditMemo>, Or<Current<ARTran.tranType>, Equal<ARDocType.debitMemo>>>>>>
						>>>>),
		...
		[PXDefault(typeof(Coalesce<Search<InventoryItem.salesAcctID, Where<InventoryItem.inventoryID, Equal<Current<ARTran.inventoryID>>>>, Search<Location.cSalesAcctID, Where<Location.locationID, Equal<Current<ARRegister.customerLocationID>>>>>))]
		...
		[PXDefault(typeof(Coalesce<Search<PMAccountTask.taskID, Where<PMAccountTask.projectID, Equal<Current<ARTran.projectID>>, And<PMAccountTask.accountID, Equal<Current<ARTran.accountID>>>>>,
			Search<PMTask.taskID, Where<PMTask.projectID, Equal<Current<projectID>>, And<PMTask.isDefault, Equal<True>, And<PMTask.type, NotEqual<ProjectTaskType.cost>>>>>>),
			PersistingCheck = PXPersistingCheck.Nothing)]
		[ActiveProjectTask(typeof(ARTran.projectID), BatchModule.AR, DisplayName = "Project Task", AlwaysVisibleIfProjectType = true)]
		...
		[PXDecimal]
		[PXDBCalced(typeof(Add<Mult<Sub<ARTran.tranAmt, ARTran.groupDiscountAmount>, Sub<decimal1, ARTran.documentDiscountRate>>,
			Mult<Sub<ARTran.tranAmt, ARTran.groupDiscountAmount>, Sub<decimal1, ARTran.origDocumentDiscountRate>>>), typeof(Decimal))]
		...
		[PXDecimal]
		[PXDBCalced(typeof(Mult<Switch<Case<Where<ARTran.drCr, Equal<DrCr.debit>>, Minus<decimal1>>, decimal1>,
			Switch<Case<Where<ARTran.taxableAmt, Equal<decimal0>>,
				Sub<Sub<Sub<ARTran.grossSalesAmount, ARTran.discAmt>, ARTran.groupDiscountAmount>, ARTran.documentDiscountAmount>>,
				ARTran.taxableAmt>>),
			typeof(Decimal))]
		...
		[IN.SubItem(
			...
			typeof(LeftJoin<INSiteStatusByCostCenter,
				...
				And<INSiteStatusByCostCenter.inventoryID, Equal<Optional<ARTran.inventoryID>>,
				And<INSiteStatusByCostCenter.siteID, Equal<Optional<ARTran.siteID>>,
		...
		[IN.LocationAvail(typeof(ARTran.inventoryID),
			typeof(ARTran.subItemID),
			typeof(CostCenter.freeStock),
			typeof(ARTran.siteID),
			typeof(Where<ARTran.tranType.IsIn<ARDocType.invoice, ARDocType.debitMemo, ARDocType.cashSale>
				.And<ARTran.invtMult.IsIn<short1, shortMinus1>>>),
			typeof(Where<ARTran.tranType.IsIn<ARDocType.creditMemo, ARDocType.cashReturn>
				.And<ARTran.invtMult.IsIn<short1, shortMinus1>>>),
			typeof(Where<False>))]
		[PXDefault(PersistingCheck = PXPersistingCheck.Nothing)]
		...
		[SO.GraphExtensions.SOInvoiceEntryExt.SOInvoiceLineSplittingExtension.ARLotSerialNbr(typeof(ARTran.inventoryID), typeof(ARTran.subItemID), typeof(ARTran.locationID), PersistingCheck = PXPersistingCheck.Nothing)]
		...
		[SO.GraphExtensions.SOInvoiceEntryExt.SOInvoiceLineSplittingExtension.ARExpireDate(typeof(ARTran.inventoryID), PersistingCheck = PXPersistingCheck.Nothing)]
		...
		[PXSelector(typeof(Search<SO.SOOrder.orderNbr, Where<SO.SOOrder.orderType, Equal<Current<blanketType>>>>), ValidateValue = false)]
	...
	public class decimalPct() : PX.Data.BQL.BqlDecimal.Constant<decimalPct>(0.01m);
}
